...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('gold', function (Blueprint $table) {
            ...
            $table->foreignIdFor(GoldType::class, "gold_type_id"); // รหัสประเภท
            $table->string("weight"); // น้ำหนัก
            $table->string("pattern_type"); // ประเภทของลาย (ตัน โป่ง)
            $table->foreignIdFor(GoldPattern::class, "gold_pattern_id"); // รหัสลาย
            ...
            $table->foreignIdFor(Wholesale::class, "wholesale_id"); // รหัสร้านขายส่ง
            $table->foreignIdFor(GoldPrice::class, "buy_price_id"); // ราคาทองตอนซื้อจากร้านขายส่ง
